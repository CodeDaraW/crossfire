# Crosscheck 最终实施方案：跨 Coding Agent 交叉 Review / Rescue 系统

> 本文为 `docs/plan-gpt.md` 与 `docs/plan-claude.md` 经五轮互审（见 `docs/review-claude.md` / `docs/review-gpt.md`）后收敛的最终基线，作为实现依据。以 plan-gpt 完整骨架为主体，已并入 plan-claude 的 gate input contract 与 anti-recursion，并清理结构图/字段/重复段的一致性问题。
