...
| claude | `claude -p --permission-mode plan --allowed-tools Read,Glob,Grep` (stdin) | `claude -p --permission-mode acceptEdits` | configurable via `CROSSFIRE_CLAUDE_BIN` |
...
Capabilities are probed at runtime (`crossfire doctor`); flags are never assumed
...
crossfire's own flags. This supports private local CLI requirements without
